...
md"""
# ECA kinetics regarding particle size
**Thomas Wutzler 2022**
"""
...
```math
\begin{aligned}
d_S &= v_{max} \frac{z_S S \, z_E E_S}{k_{S E_S} \left( 1 + \frac{z_S S}{k_{S E_S}} + \frac{z_E E_S}{k_{S E_S}} + \frac{M}{k_{SM}} \right)} \\
&= v_{max} \frac{z_S S \, z_E E_S}{ k_{S E_S} + z_S S + z_E E_S + \frac{1}{k_{SM}} k_{S E_S} M}  \\
\end{aligned}
```
...
```math
\begin{aligned}
d_S &= v_{max} \frac{z_S S \, z_E E_S}{k_{S E_S} \left( 1 + \frac{z_S S}{k_{S E_S}} + \frac{z_E E_S}{k_{S E_S}} + \frac{M}{k_{SM}} + \frac{M}{k_{EM}} \right)} \\
&= v_{max} \frac{z_S S \, z_E E_S}{ k_{S E_S} + z_S S + z_E E_S + \left( \frac{1}{k_{SM}} + \frac{1}{k_{EM}} \right) k_{S E_S}M}  \\
\end{aligned}
```



All pools need to be expressed as concentration of binding sites per volume, i.e. per depth per area. Hence, we need to convert mass of $S$ and $E_S$ to concentration of binding siztes by factors $z_S$ and $z_E$.
"""

# ╔═╡ 3753395d-1762-4bd3-ba08-da43a5607973
md"""
We follow Tang & Riley (2017) an assume both enzymes and substrate particles to be spherical with mean radius $R_E$ and $R_S$ respectively. Further we assume that enzmyes can only attack the surface of the particle. 
...
md"""
We assume that each enzyme has one binding site. Hence we need to convert mass to volume. 
$z_E = \frac{1}{p_s s_B \rho_E}$

Factor $s_B \le 1$ accounts for clumped microbial biomass, where biomass and enzymes are concentrated in a fraction of the volume only.
"""

# ╔═╡ c9bafcb9-1a02-4fb9-a66f-4c56a05e8736
md"""
Original chemical equailibrium refers to binding sites distributed in a solution.
...
Hence with the same reference volume we take $\rho_S = \frac{S}{V_{ref}} = 0.1 S m^{-3}$ and $\rho_E = 0.1 E_S m^{-3}$.
