...
function parameter_input(pars_mod)
	return PlutoUI.combine() do Child
		inputs = [begin
			...
			el = Scrubbable(range(row.lower, row.upper, length = 100);default=row.default, format = ".3")
...
plot(sol0t, tspan = (tstart, tend), vars=[s.dec_R p_upd[symbol(s.ϵ_tvr)] * s.tvr_B + (1-p_upd[symbol(s.κ_E)])*s.tvr_Enz], legend=:bottomright); vline!([0], linestyle=:dash, color = :lightgray, label=nothing)

# ╔═╡ 2448b012-864d-4797-b768-33174f9fd08f
plot(sol0t, tspan = (tstart, tend), vars=[s.α_L,s.α_R], legend=:bottomright); vline!([0], linestyle=:dash, color = :lightgray, label=nothing)

# ╔═╡ 773273a8-70fc-4897-8174-8b06aff47013
plot(sol0t, tspan = (tstart, tend), vars=[s.lim_C, s.lim_N]); vline!([0], linestyle=:dash, color = :lightgray, label=nothing)

# ╔═╡ 0cbe8944-5ebd-4804-863d-e567c703b89f
plot(sol0t, tspan = (tstart, tend), vars=[s.I_N]); vline!([0], linestyle=:dash, color = :lightgray,label=nothing)
