...
function som_change(sol)
    !(sol.retcode ∈ (:Success, :Terminated)) &&
        return ((; csom0 = missing, csomF = missing, csomD = missing,
...
i_outputLatex = () -> begin
    ...
    tmp = @chain df_dist_opt begin
        ...
        # getproperty(:pars)
        # join("\",\"")
        transform(:dist => ByRow(d -> (Base.typename(typeof(d)).name, d.μ, d.σ, quantile(d, 0.025))) => [
            :typename,
            :μ,
            :σ,
            :lower,
    ...
    tmp2 = @chain tmp begin
        transform([:lower, :mode, :upper] => ByRow((x...) -> round.(x, sigdigits = 2)) => [
            :lower,
            :mode,
            :upper,
        ])
...
levels!(dfp.par,
    subset(dfp,
        AsTable([:target, :index]) => ByRow(x -> values(x) == ("ΔSOM", "total"))).par);
