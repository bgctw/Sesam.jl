...
k_R_app = p[s.k_R]
p[s.k_R] = p[s.k_L] # same rate at litter
p[s.K_eqR] = p[s.k_R] / k_R_app / p[s.Q_max]
...
i_plot = () -> begin
    #using Plots
    plot(sol)
    plot(sol, vars = [s.R, s.Ra, s.L])
    plot(sol, vars = [s.R / s.Ra])
    plot(sol, vars = [1 / s.k_Rtot])
    plot(sol, vars = [s.lim_C, s.lim_N])
    plot(sol, vars = [s.α_L, s.α_R])
    plot(sol, vars = [s.R / s.R_N]) # stays the same
end
